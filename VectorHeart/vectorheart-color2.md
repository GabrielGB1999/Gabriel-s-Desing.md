...
## Colors
The palette replaces heat with frequency — a deep midnight field sliced by electric cyan, the visual language of radar screens, sonar returns, and ultraviolet signal.

- **Neutral (#040810):** Deep midnight. The ground is almost pure darkness but carries a barely perceptible cold blue cast — the darkness of deep water rather than empty space.
- **Primary (#00C8E8):** Electric cyan. The stripe bundle's signal color. High-frequency, clinical, and relentless. It reads equally as technological precision and kinetic energy.
- **Secondary (#EEF4F8):** Cold ice white. Flanking slabs and hero typography. Its blue-cool temperature reinforces the frequency character of the primary without competing with it.
- **Tertiary (#FF5200):** Alarm orange. The accent plane counterweight. The only warm element in the system, placed as a single angular strike against the cold cyan field — maximum chromatic opposition.
- **Steel (#445C72):** Slate steel. All technical labels and structural marks. Pulled from the blue-gray range that sits between the primary and the neutral, present everywhere but never dominant.
- **Accent (#8800CC):** Electric violet. Reserved for the single corner accent element. Its ultraviolet character connects to the cyan's high-frequency energy while introducing a distinct spectral territory.

The `glow` token (#007A90) is a deeper, desaturated cyan used exclusively for the multi-layer bloom behind the stripe bundle. It should never appear as a solid fill in a visible element.
## Typography
